...
const ENCABEZADOS = [
  ...
  'FOLLETOS','INVITADA','LECTURA DE LIBRO','OTROS', 'FECHA','INDICE DESDE','INDICE HASTA','LIBRO','ESTADO LIBRO',
...
function doPost(e) {
  ...
  try {
    ...
    const versiculo = (Number(d.versos) > 0) ? (VERSICULOS_MES[d.mesConsigna] || d.mesConsigna) : 0;
    hoja.appendRow([
      d.nombre,                    // A NOMBRE
...
function guardarFirma_(dataUrl, nombre, fecha) {
  ...
  const archivo = DriveApp.createFile(
    Utilities.newBlob(bytes, 'image/png', 'firma_' + fecha + '_' + String(nombre).replace(/[^\wÁÉÍÓÚÑáéíóúñ ]/g, '') + '.png')
  );
